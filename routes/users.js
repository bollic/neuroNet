...
async function ensureAdminExists(req, res, next) {
  try {
    ...
    if (!existingAdmin) {
      console.log("⚠️ Nessun admin trovato nel DB");
    }
...
router.get('/users', isAdmin, async (req, res) => {
  try {
    ...
    const groupsFromDb = await Group.find().lean();

    const groupedUsers = {};

users.forEach(user => {

  const key = user.groupId || 'no-group';
   // 🔥 trova il nome del gruppo
  const groupData = groupsFromDb.find(
    g => g.groupId === user.groupId
  );

  if (!groupedUsers[key]) {
    
    groupedUsers[key] = {
      groupName: groupData?.name || key,
  ...
  if (user.role === 'office') {
    groupedUsers[key].office = user;
  }
  else if (user.role === 'field') {
    groupedUsers[key].fields.push(user);
  }
  else {
    groupedUsers[key].others.push(user);
  }
